...
var queryString = window.location.search.substring(1);
    ...
    // Bouclez à travers chaque paire clé-valeur et ajoutez-les à l'objet
    for (var i = 0; i < params.length; i++) {
       ...
       Object.assign(paramObject, {key: value});

    }

  //var id_line =  paramObject.pop();

     // id_line = id_line.substring(4);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            if ($.trim(data.attribut_champ)) {
                ...
                $.each(data.attribut_champ, function() {
                    if (this.type_champs == "Text") {
                        ...
                        if (params !== undefined){
                            ...
                            if( position_item != -1 ){
                                row_select1 += ' <input class="form-control" type="text" value="'+item_value[position_item]+'" id="Objet_file" name="valeur[]" required>';
                ...
                if ( p_id_line != -1 ){
                    row_select1 += '<input type="text" value="'+item_value[p_id_line].substring(4)+'" hidden name="id_line">';
                }
            ...
            success: function(data) {
                ...
                switch(id_view) {
                    case "1":
                      ...
                      break;
                    case "2":
                        window.location.href = APP_URL+ "/recherche_dossier_choix";
                      break;
                    default:
