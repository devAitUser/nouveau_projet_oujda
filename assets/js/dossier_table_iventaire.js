...
$(document).ready(function() {
  ...
  // Gérer le clic sur le bouton dans chaque ligne
  $('#table tbody').on('click', '.btnGetRowData', function () {
      ...
      var lien = '/create_dossier?'

      for (var i = 0; i < data.length; i++) {
  
        lien +=array_champs[i]+'='+data[i]+'&';
      ...
      lien +=  "DT_RowId="+data['DT_RowId'];
      


        window.open(APP_URL+lien);
    
      // Utilisez "rowData" comme bon vous semble
  });
   ...
   $('#table tbody').on('click', '.btn_edit', function () {
        ...
        data.pop();
            ...
            for (var i = 0; i < data.length; i++) {
              ...
              row_select1 = '<div class="form-group col-md-6">';
              row_select1 += '<label for="inputEmail4">'+array_champs[i]+'</label>';
              row_select1 += ' <input type="text" name="value[]" class="form-control" id="" value='+data[i]+' placeholder="">';
              row_select1 += ' </div> ';

              
      
  
              $(".parent_column").append(row_select1);
